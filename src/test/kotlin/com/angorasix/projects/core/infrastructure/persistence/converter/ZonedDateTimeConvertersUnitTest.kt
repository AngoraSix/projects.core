...
class ZonedDateTimeConvertersUnitTest {
    ...
    fun givenDocumentWithCorrectFields_whenReadingDocument_thenZonedDateTimeCorrectlyCreated() {
        val baseZdt = ZonedDateTime.of(1991, 4, 22, 23, 0, 0, 0, ZoneId.of("America/Argentina/Cordoba"))
        val inputDocumentMap: MutableMap<String, Any> = HashMap()
        inputDocumentMap["zone"] = "America/Argentina/Cordoba"
        inputDocumentMap["dateTime"] = Date.from(baseZdt.withZoneSameInstant(ZoneId.of("UTC")).toInstant())
        val inputDocument = Document(inputDocumentMap)
        val outputTdz = readerConverter.convert(inputDocument)
        Assertions.assertThat(outputTdz).isEqualTo(baseZdt)
        Assertions.assertThat(outputTdz.toString()).isEqualTo("1991-04-22T23:00-04:00[America/Argentina/Cordoba]")
    }
}